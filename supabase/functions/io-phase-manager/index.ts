...
async function handleEvaluate(supabase: any, userId: string) {
  ...
  await supabase.from("observability_logs").insert({
    ...
    event_data: {
      ...
      diary_entries_analyzed: (diarySignals as any)?.total_entries ?? null,
      diary_avg_genuineness: (diarySignals as any)?.avg_genuineness ?? null,
      diary_engagement: (diarySignals as any)?.diary_engagement ?? null,
      diary_dominant_tone: (diarySignals as any)?.dominant_tone ?? null,
      shadow_mode: true,
    },
